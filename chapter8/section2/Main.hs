...
mkParams = Params <$> argument str (metavar "FILE" <> help "IP range database")
...
main = (execParser opts >>= run)
  ...
  where
    opts =
        ...
        (fullDesc <>
            progDesc ("Answers YES/NO depending on whether " ++"an IP address belongs to the IP range database"))
