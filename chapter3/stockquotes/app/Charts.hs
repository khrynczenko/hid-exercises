...
plotChart title quotes filePath = do
    _ <- renderableToFile fileOptions filePath (toRenderable chart)
    pure ()
  where
    fileOptions = FileOptions (800, 600) SVG loadSansSerifFonts

    (candles, closings, volumes) = unzip3 $
        [ (Candle (day quote) (low quote) (open quote) 0 (close quote) (high quote),
          (day quote, close quote),
          (day quote, [volume quote])) | quote <- toList quotes ]
